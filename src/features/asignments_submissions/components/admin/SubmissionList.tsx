...
import type { Submission, Assignment } from "../../types/assignment";
import {
  CheckCircleIcon,
  ClockIcon,
...
export const SubmissionList: React.FC<SubmissionListProps> = ({
  ...
  const filteredSubmissions = submissions.filter((submission) => {
    ...
    const assignmentMatch =
      ...
      submission.assignmentId === assignmentFilter;
      ...
      <div className="divide-y divide-gray-200">
        {filteredSubmissions.map((submission) => {
          const {
            ...
            label: statusLabel,
          } = getStatusConfig(submission.status);
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-3 sm:space-y-0">
                <div className="flex-1 min-w-0">
                  <div className="flex items-start space-x-3">
                    <div
                      className={`p-2 rounded-full ${statusBgColor} flex-shrink-0`}
                    >
                      <StatusIcon
                        className={`h-4 w-4 sm:h-5 sm:w-5 ${statusColor}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0 sm:pl-4 sm:min-w-max">
                  ...
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${statusBgColor} ${statusColor} w-fit`}
                  >
                    {statusLabel}
                  </span>
                  <button
