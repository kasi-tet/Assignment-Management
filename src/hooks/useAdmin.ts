...
import { DashboardStats } from '../../types/assignment';
...
export const useAdmin = () => {
  ...
  const dashboardStats = useMemo((): DashboardStats => {
    ...
    const pendingReviews = submissions.filter(s => s.status === 'pending' || s.status === 'under-review').length;
    const completedReviews = submissions.filter(s => s.status === 'approved' || s.status === 'rejected').length;
  ...
  const getAssignmentAnalytics = (assignmentId: string) => {
    ...
    return {
      ...
      averageGrade: assignmentSubmissions.filter(s => s.grade).reduce((acc, curr) => acc + (curr.grade || 0), 0) / assignmentSubmissions.filter(s => s.grade).length || 0
    };
  };
  ...
  const getMemberPerformance = (memberId: string) => {
    ...
    const averageGrade = approvedSubmissions.reduce((acc, curr) => acc + (curr.grade || 0), 0) / approvedSubmissions.length || 0;
    
    return {
      totalSubmissions: memberSubmissions.length,
      approvedSubmissions: approvedSubmissions.length,
      averageGrade,
      completionRate: assignments.length > 0 ? (memberSubmissions.length / assignments.filter(a => a.assignedTo?.includes(memberId)).length) * 100 : 0
    };
  };
